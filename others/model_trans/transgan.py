...
from model_trans.vit_helper import DropPath, trunc_normal_
...
class Generator(nn.Module):
    def __init__(self, args, embed_dim=384,
...
def get_optimizer(args, m):
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, m.parameters()), args.lr)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, "min", threshold=1e-4)

    return optimizer, scheduler


def optimize_parameters(optimizer, scheduler, m, x, y, mask):
    # forward
    optimizer.zero_grad()
    out = m.forward(x)
    loss = compute_loss(out * mask, y * mask)
    loss.backward()
    optimizer.step()
    scheduler.step(loss)
